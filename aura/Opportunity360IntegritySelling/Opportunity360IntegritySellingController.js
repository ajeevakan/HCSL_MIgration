({
 ...
 // $A.enqueueAction(action3);
    ...
    handleMenuSelect: function(cmp, event, helper) {
        ...
        var myArray = selectedMenuItemValue.split(",");
        if (myArray[1]=="MakePrimaryContact"){
            helper.callMakePrimaryContact(cmp,myArray[0]);
        }
        else if (myArray[1]=="MakePrimaryResident"){
            helper.callMakePrimaryResident(cmp,myArray[0]);
        }
        else if (myArray[1]=="Reactivate"){
            helper.callSetContactInactive(cmp,myArray[0],false,false,true);
        }
        else if (myArray[1]=="Inactivate"){
            helper.callSetContactInactive(cmp,myArray[0],true,false,false);
        }
        else if (myArray[1]=="InactivateD"){
            helper.callSetContactInactive(cmp,myArray[0],true,true,true);
        }
        else if (myArray[1]=="MakeSecondResident"){
            helper.callMakeSecondResident(cmp,myArray[0],true,true,true);
        }
        else if (myArray[1]=="DemoteSecondResident"){
            helper.callDemoteSecondResident(cmp,myArray[0],true,true,true);
        }        
